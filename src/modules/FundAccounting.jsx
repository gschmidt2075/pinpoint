...
export default function FundAccounting({ db, dispatch }) {
  const [view, setView] = useState("dashboard"); // dashboard | newExpenditure | newRevenue | ledger | amendments

  return (
    <div>
      {/* Sub-nav */}
      <div style={{ display: "flex", gap: 2, marginBottom: 24, borderBottom: "1px solid #ddd", paddingBottom: 0 }}>
        {[
      ...
      {view === "newExpenditure" && <ExpenditureForm dispatch={dispatch} onDone={() => setView("ledger")} />}
...
function FADashboard({ db }) {
  ...
  // Aggregate expenditures by account code
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 22 }}>
        ...
        <KPICard label="Pending Approvals" value={pendingCount} sub="Awaiting review" accent="#d97706" />
      </div>
        ...
        {categories.map(cat => {
          ...
          return (
            <button key={cat} onClick={() => setCatFilter(active ? "all" : cat)} style={{
              ...
              <div style={{ fontSize: 15, fontWeight: 700, fontFamily: "monospace" }}>{fmt(budg)}</div>
      ...
      <SectionCard
        ...
        subtitle="Click a category above to filter"
        action={
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…"
            style={{ ...inp, width: 200, margin: 0 }} />
        }
      >
        <div style={{ maxHeight: 420, overflowY: "auto" }}>
          <Table
            headers={[
              { label: "Code" }, { label: "Description" }, { label: "Fund" },
            ...
            rows={filtered.map(c => {
              ...
              return [
                ...
                c.description,
                <span style={{ background: "#e8f0fb", color: "#1a4a8a", padding: "2px 7px", borderRadius: 4, fontSize: 11, fontWeight: 600 }}>Roads</span>,
...
// ── Expenditure Form — supports split across multiple codes + funds ────────────
function ExpenditureForm({ dispatch, onDone }) {
  const emptyLine = () => ({ id: Date.now() + Math.random(), code: "", fund: "ROADS", amount: "" });
  const [header, setHeader] = useState({
    date: "", vendor: "", type: "invoice", reference: "", description: "", status: "pending",
  });
  ...
  const handleSubmit = () => {
    if (!header.date || !header.vendor || lines.some(l => !l.code || !l.amount)) return;
    dispatch({ type: "ADD_EXPENDITURE", payload: {
      id: Date.now(), ...header,
      lines: lines.map(l => ({ code: l.code, fund: l.fund, amount: parseFloat(l.amount) })),
      totalAmount,
    }});
    setHeader({ date: "", vendor: "", type: "invoice", reference: "", description: "", status: "pending" });
    setLines([emptyLine()]);
    setSaved(true);
  ...
  return (
    <div style={{ maxWidth: 800 }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: "#888", marginTop: 3 }}>Split across multiple account codes and funds as needed</div>
      </div>

      {saved && <div style={{ background: "#e6f4ec", border: "1px solid #a8d5b5", borderRadius: 6, padding: "12px 16px", marginBottom: 16, color: "#1a6b35", fontWeight: 600, fontSize: 13 }}>✓ Expenditure recorded — redirecting to ledger…</div>}

      {/* Header */}
      <div style={{ background: "#fff", border: "1px solid #ddd", borderRadius: 8, padding: 22, marginBottom: 16 }}>
        <div style={{ fontWeight: 700, fontSize: 13, color: "#1a1a1a", marginBottom: 14 }}>Transaction Header</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
          ...
          <Field label="Type">
            <select value={header.type} onChange={e => setH("type", e.target.value)} style={inp}>
              {EXP_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
          </Field>
          <Field label="Reference #"><input type="text" placeholder="Invoice / PO number…" value={header.reference} onChange={e => setH("reference", e.target.value)} style={inp} /></Field>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <Field label="Vendor / Payee" required><input type="text" placeholder="Vendor name…" value={header.vendor} onChange={e => setH("vendor", e.target.value)} style={inp} /></Field>
          <Field label="Description"><input type="text" placeholder="Brief description…" value={header.description} onChange={e => setH("description", e.target.value)} style={inp} /></Field>
        </div>
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid #ddd", borderRadius: 8, padding: 22, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div style={{ fontWeight: 700, fontSize: 13, color: "#1a1a1a" }}>Account Code Lines</div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {["all", ...categories].map(cat => (
              <button key={cat} onClick={() => setCatFilter(cat)} style={{
                padding: "3px 10px", fontSize: 11, fontWeight: 600, textTransform: "capitalize",
        ...
        {lines.map((line, i) => {
          ...
          return (
            <div key={line.id} style={{ background: "#fafaf8", border: "1px solid #eee", borderRadius: 6, padding: "14px", marginBottom: 10 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                <div style={{ flex: 2 }}>
                  <Field label={`Account Code ${i + 1}`} required>
                ...
                <div style={{ flex: 1 }}>
                  <Field label="Amount ($)" required>
                    <input type="number" min="0" step="0.01" placeholder="0.00" value={line.amount} onChange={e => setL(line.id, "amount", e.target.value)} style={{ ...inp, fontFamily: "monospace" }} />
                  </Field>
                </div>
                {lines.length > 1 && (
                  <button onClick={() => removeLine(line.id)} style={{ ...btn.danger, marginTop: 22, padding: "9px 12px", fontSize: 16, lineHeight: 1 }}>×</button>
                )}
              </div>
            </div>
          );
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
          <button onClick={addLine} style={{ ...btn.secondary, fontSize: 12, padding: "7px 14px" }}>+ Add another code</button>
          <div style={{ fontFamily: "monospace", fontSize: 16, fontWeight: 700, color: "#1a1a1a" }}>
            Total: {fmtSm(totalAmount)}
          </div>
        </div>
      </div>

      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={handleSubmit} style={btn.primary}>Submit for Approval</button>
        <button onClick={() => { setHeader({ date:"",vendor:"",type:"invoice",reference:"",description:"",status:"pending" }); setLines([emptyLine()]); }} style={btn.ghost}>Clear</button>
      </div>
    </div>
  );
}

// ── Revenue Form — supports multiple sources / codes ──────────────────────────
  ...
  return (
    <div style={{ maxWidth: 800 }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: "#888", marginTop: 3 }}>Record grants, permits, intergovernmental transfers, and other receipts</div>
      </div>

      {saved && <div style={{ background: "#e6f4ec", border: "1px solid #a8d5b5", borderRadius: 6, padding: "12px 16px", marginBottom: 16, color: "#1a6b35", fontWeight: 600, fontSize: 13 }}>✓ Revenue recorded — redirecting to ledger…</div>}

      <div style={{ background: "#fff", border: "1px solid #ddd", borderRadius: 8, padding: 22, marginBottom: 16 }}>
        <div style={{ fontWeight: 700, fontSize: 13, color: "#1a1a1a", marginBottom: 14 }}>Transaction Header</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
          <Field label="Date" required><input type="date" value={header.date} onChange={e => setH("date", e.target.value)} style={inp} /></Field>
          <Field label="Reference #"><input type="text" placeholder="Check / warrant number…" value={header.reference} onChange={e => setH("reference", e.target.value)} style={inp} /></Field>
          <Field label="Status">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <Field label="Source / Grantor / Payer" required><input type="text" placeholder="e.g. IDOT, FHWA, Township of…" value={header.source} onChange={e => setH("source", e.target.value)} style={inp} /></Field>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {["all", ...revTypes].map(t => (
              <button key={t} onClick={() => setTypeFilter(t)} style={{
                padding: "3px 10px", fontSize: 11, fontWeight: 600, border: "1px solid", borderRadius: 4, cursor: "pointer",
...
function Ledger({ db, dispatch }) {
  ...
  const updateStatus = (tx, status) => {
    dispatch({ type: tx.txType === "expenditure" ? "UPDATE_EXP_STATUS" : "UPDATE_REV_STATUS", payload: { id: tx.id, status } });
  };
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 18, flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <div style={{ display: "flex", border: "1px solid #ccc", borderRadius: 6, overflow: "hidden" }}>
            {["all","expenditure","revenue","pending"].map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: "8px 12px", fontSize: 12, fontWeight: 600, border: "none", cursor: "pointer", background: filter === f ? "#1a3a5c" : "#fff", color: filter === f ? "#fff" : "#555", textTransform: "capitalize" }}>{f}</button>
            ))}
          </div>
        </div>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ background: "#f7f7f5" }}>
              {["Date","Type","Vendor / Source","Codes","Total","Status","Actions"].map(h => (
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={7} style={{ padding: "36px", textAlign: "center", color: "#aaa", fontSize: 13 }}>
                {allTx.length === 0 ? "No transactions yet — use New Expenditure or New Revenue to add one." : "No transactions match your filter."}
              </td></tr>
            )}
            {filtered.map((t, i) => (
              <tr key={t.id + t.txType} style={{ borderTop: "1px solid #eee", background: i % 2 === 0 ? "#fff" : "#fafaf8" }}>
                ...
                <td style={{ padding: "10px 14px", fontWeight: 600, color: "#1a1a1a", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.vendor}</td>
                <td style={{ padding: "10px 14px" }}>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                    {(t.lines||[]).map((l, j) => (
                      <span key={j} title={getCodeDesc(l.code)} style={{ background: "#f0f0ee", color: "#1a3a5c", padding: "1px 6px", borderRadius: 3, fontSize: 10, fontFamily: "monospace", fontWeight: 600 }}>
                        {l.code} {fmtSm(l.amount)}
                      </span>
                    ))}
                  </div>
                </td>
                <td style={{ padding: "10px 14px", textAlign: "right", fontFamily: "monospace", fontWeight: 700, color: t.txType === "revenue" ? "#1a6b35" : "#1a1a1a" }}>
                  {t.txType === "revenue" ? "+" : ""}{fmtSm(t.totalAmount)}
                </td>
                <td style={{ padding: "10px 14px" }}><StatusBadge status={t.status} /></td>
                <td style={{ padding: "10px 14px" }}>
                  {t.status === "pending" && (
                    <div style={{ display: "flex", gap: 4 }}>
                      <button onClick={() => updateStatus(t, "approved")} style={{ ...btn.small, background: "#1a6b35", fontSize: 10, padding: "4px 8px" }}>Approve</button>
                      <button onClick={() => updateStatus(t, "voided")} style={{ ...btn.small, background: "#c0392b", fontSize: 10, padding: "4px 8px" }}>Void</button>
                    </div>
                  )}
                  {t.status === "approved" && (
                    <button onClick={() => updateStatus(t, "posted")} style={{ ...btn.small, background: "#1a4a8a", fontSize: 10, padding: "4px 8px" }}>Post</button>
                  )}
                </td>
              </tr>
            ))}
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: "#888", marginTop: 3 }}>Increase or decrease an account code's appropriation mid-year. Use negative amounts to reduce.</div>
      </div>
          ...
          <Field label="Account Code" required>
            <select value={form.code} onChange={e => set("code", e.target.value)} style={{ ...inp, fontFamily: "monospace", fontSize: 12 }}>
              ...
              {EXPENDITURE_CODES.filter(c => c.budgeted > 0).map(c => (
                <option key={c.code} value={c.code}>{c.code} — {c.description} (${c.budgeted.toLocaleString()})</option>
              ))}
            </select>
          </Field>
        </div>
